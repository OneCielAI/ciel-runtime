"""Request-scoped Aliyun CAPTCHA headers for the official ZCode Start Plan."""
...
class ZaiStartPlanCaptchaConfig:
    ...
    def from_envelope(cls, envelope: Any) -> "ZaiStartPlanCaptchaConfig":
        ...
        if not result.region or not result.prefix or not result.scene_id:
...
class _CaptchaResultReceiver:
    ...
    def url(self) -> str:
        ...
        return f"{self._public_origin(self._server.server_port)}{_CAPTCHA_PATH}?{query}"

    def _public_origin(self, server_port: int) -> str:
    ...
    def _valid_state(self, query: str) -> bool:
        supplied = str(
            (urllib.parse.parse_qs(query, keep_blank_values=True).get("state") or [""])[0]
        )
...
<style>body{{font:16px system-ui,sans-serif;max-width:680px;margin:48px auto;padding:0 24px;color:#17202a}}
#captcha-element{{min-height:1px}}button{{font:inherit;padding:10px 16px}}#status{{white-space:pre-wrap}}.muted{{color:#5f6b76}}</style>
</head><body><h1>Z.AI Start Plan verification</h1>
...
<script>const CIEL_CAPTCHA={config_json};</script><script src="{sdk_url}"></script>
<script>(()=>{{
const status=document.getElementById('status'),button=document.getElementById('captcha-button');
let instance=null,submission=null,interactive=false;
const setStatus=v=>status.textContent=v;
const verifyCode=v=>v&&typeof v==='object'?String(v.verifyCode||v.VerifyCode||''):'';
const terminalPass=v=>!!(v&&typeof v==='object'&&((v.success===true&&v.verifyResult===true)||verifyCode(v)==='T006'));
const param=v=>v&&typeof v==='object'?String(v.captchaVerifyParam||v.CaptchaVerifyParam||'').trim():'';
const submit=value=>{{if(submission)return submission;const text=String(value||'').trim();if(!text)throw new Error('CAPTCHA returned an empty verification result.');submission=(async()=>{{const q=new URLSearchParams({{state:CIEL_CAPTCHA.state}});const r=await fetch(`${{CIEL_CAPTCHA.resultPath}}?${{q}}`,{{method:'POST',headers:{{'Content-Type':'text/plain;charset=UTF-8'}},body:text}});if(!r.ok)throw new Error(`Ciel Runtime rejected the result (${{r.status}}).`);setStatus('Verification complete. Returning to Ciel Runtime…');button.hidden=true;setTimeout(()=>window.close(),700)}})();return submission}};
const show=()=>{{interactive=true;setStatus('Complete the verification challenge to continue the model request.');if(instance&&typeof instance.show==='function')instance.show();else button.click()}};
button.addEventListener('click',()=>{{if(instance&&typeof instance.show==='function')instance.show()}});
window.AliyunCaptchaConfig={{region:CIEL_CAPTCHA.region,prefix:CIEL_CAPTCHA.prefix}};
if(typeof window.initAliyunCaptcha!=='function'){{setStatus('The official Aliyun CAPTCHA SDK could not be loaded.')}}else window.initAliyunCaptcha({{
SceneId:CIEL_CAPTCHA.sceneId,mode:'popup',language:'en',showErrorTip:false,element:'#captcha-element',button:'#captcha-button',
getInstance:value=>{{instance=value;setTimeout(()=>{{setStatus('Running security verification…');if(typeof value.startTracelessVerification==='function')value.startTracelessVerification();else show()}},2000)}},
success:value=>submit(value).catch(e=>setStatus(e.message)),
fail:value=>{{const p=param(value);if(terminalPass(value)&&p){{submit(p).catch(e=>setStatus(e.message));return}}if(verifyCode(value)==='F008'){{setStatus('Verification result was already used. Select Verify to retry.');return}}if(!interactive||value?.success===true&&value?.verifyResult===false)show();else setStatus('Verification was not accepted. Select Verify to try again.')}},
...
class ZaiStartPlanCaptchaBroker:
    ...
    _cached_config: tuple[float, ZaiStartPlanCaptchaConfig] | None = field(
        default=None, init=False
    )

    def fetch_config(self, app_version: str) -> ZaiStartPlanCaptchaConfig:
        now = self.clock()
        if self._cached_config is not None and self._cached_config[0] > now:
            return self._cached_config[1]
        query = urllib.parse.urlencode(
            {"app_version": app_version, "platform": self._platform_key()}
        )
        ...
        result = ZaiStartPlanCaptchaConfig.from_envelope(envelope)
        self._cached_config = (now + 60.0, result)
        return result

    def headers(self, options: Mapping[str, Any]) -> dict[str, str]:
        app_version = str(options.get("zcode_app_version") or "3.9.1").strip()
        ...
        with self._request_lock:
            ...
            with self.receiver_factory(
                ...
                state,
                timeout,
                host=self._bind_host(options),
                port=self._port(options),
                public_base_url=self._public_base_url(options),
            ) as receiver:
                event = self._publish_pending(state, receiver.url, timeout)
                self.log("INFO", f"zai_start_plan_captcha_waiting url={receiver.url}")
                try:
                    opened = False
                    try:
                        opened = bool(self.open_url(receiver.url))
                    ...
                    if not opened and not self._public_base_url(options):
                        raise RuntimeError(
                    ...
                    self._publish_status(event, "failed", str(exc))
                    raise
                self._publish_status(event, "completed")
        self.log(
            "INFO",
    ...
    def _publish_pending(
        self, state: str, url: str, timeout: float
...
class ZaiStartPlanRuntimeHeaderPreparer:
    ...
    def __call__(
        ...
        projected = {
            ...
            if name.casefold()
            not in {
                CAPTCHA_PARAM_HEADER.casefold(),
                CAPTCHA_REGION_HEADER.casefold(),
                SESSION_ID_HEADER.casefold(),
            }
        }
        if str(provider or "").casefold() == "zai-start-plan":
            projected.update(self.broker.headers(config))
        return projected


def zai_start_plan_runtime_header_services(
    interaction_path: Any,
    log: Callable[[str, str], None],
) -> tuple[RuntimeInteractionRepository, ZaiStartPlanRuntimeHeaderPreparer]:
    interactions = RuntimeInteractionRepository(interaction_path, log=log)
    return interactions, ZaiStartPlanRuntimeHeaderPreparer(
        log=log,
...
__all__ = [
    ...
    "zai_start_plan_runtime_header_services",
]
