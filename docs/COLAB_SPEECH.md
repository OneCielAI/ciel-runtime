...
MOSS-TTS-Nano is a voice-cloning model without built-in speakers. Deployment configures the project's official `zh_1.wav` sample so the first request works immediately. In Web Chat > Speech Settings, upload a reference voice clip to replace it. The decoded transport/storage safety limit defaults to its configurable maximum of 500 MiB; it is not a provider-semantic recommendation, and the browser displays the active server limit. Ciel decodes an uploaded data URL once into an atomic private binary sidecar under `CONFIG_DIR/tts-reference-audio`, stores only an opaque marker plus the exact transcript in `config.json`, and never exposes either audio data or the marker through the public configuration response. It expands the marker only while forwarding a TTS request and reserves the resulting JSON growth in the router's shared in-flight budget. Clearing or replacing the reference removes the old sidecar; legacy embedded data URLs migrate on the next settings save, while HTTP(S) reference URLs remain unchanged. API clients can instead pass `ref_audio` as an HTTP(S) URL or base64 audio data URL to `POST /v1/audio/speech`.

The batch speech schema can carry a shared top-level `ref_audio` or per-item `ref_audio` values. Ciel validates base64 data URLs in both locations against the decoded reference limit. The `/v1/audio/speech/batch` aggregate wire cap is the provider-neutral technical ceiling `4 GiB / 5` (about 819.2 MiB). It fits one maximum 500 MiB reference after base64 JSON expansion and keeps the uniform five-times memory reservation within the 4 GiB in-flight maximum without assuming a provider-specific item count.
