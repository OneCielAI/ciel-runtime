...
실행 중 LLM option의 최초 snapshot, restore, preset 적용, slider 이동, status/list projection을 조정하는 Application Controller. 설정 저장, presentation, preset mutation을 각각 7필드 이하 포트로 분리하며 snapshot은 JSON-compatible 깊은 복사 의미를 유지한다.
