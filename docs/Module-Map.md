...
메인 호환 엔트리포인트이자 composition root. 라우터 조정, 제공자 설정, CLI 및 레거시 변환의 공개 이름을 보존하되, 시그니처가 완전히 같은 무상태 함수는 support 구현을 직접 re-export하고 orchestration만 typed service로 조립한다.
