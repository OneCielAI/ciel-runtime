...
Anthropic, Ollama, OpenRouter, LM Studio, vLLM, NVIDIA NIM, DeepSeek, Kimi, Z.AI, Fireworks, OpenCode 등 Provider별 구체 Adapter와 Registry. 각 Adapter가 인증, API-key 상태·launch blocker, configuration/context capability, protocol, endpoint, 모델-ID 정규화 및 모델 discovery 경로를 소유한다.
...
Provider Adapter의 `ProviderContextPolicy`에 따라 모델 컨텍스트 용량 탐색 순서와 설정 상한 적용을 수행하는 Provider-neutral 정책 서비스. 중앙 Provider 이름 분기 없이 managed/Ollama/standard 설정 전략을 실행한다.
