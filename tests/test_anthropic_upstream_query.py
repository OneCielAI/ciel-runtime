...
class InboundBetaFlagTests(unittest.TestCase):
    def test_beta_true_detected(self):
        self.assertTrue(ciel_runtime.inbound_query_has_beta_flag("/v1/messages?beta=true"))

    def test_beta_one_detected(self):
        self.assertTrue(ciel_runtime.inbound_query_has_beta_flag("/v1/messages?beta=1"))

    def test_beta_among_other_params(self):
        self.assertTrue(
            ciel_runtime.inbound_query_has_beta_flag("/v1/messages?foo=bar&beta=true&x=2")
        )

    def test_no_query_is_false(self):
        self.assertFalse(ciel_runtime.inbound_query_has_beta_flag("/v1/messages"))

    def test_beta_false_is_false(self):
        self.assertFalse(ciel_runtime.inbound_query_has_beta_flag("/v1/messages?beta=false"))

    def test_unrelated_param_is_false(self):
        self.assertFalse(ciel_runtime.inbound_query_has_beta_flag("/v1/messages?other=true"))
...
class UpstreamMessagesQueryTests(unittest.TestCase):
    def test_inbound_beta_propagated_for_anthropic(self):
        self.assertEqual(
            "beta=true",
            ciel_runtime.upstream_messages_query({}, "/v1/messages?beta=true", "anthropic"),
        )

    def test_inbound_beta_is_not_propagated_without_anthropic_provider(self):
        self.assertEqual("", ciel_runtime.upstream_messages_query({}, "/v1/messages?beta=true"))

    def test_no_query_yields_empty(self):
        self.assertEqual("", ciel_runtime.upstream_messages_query({}, "/v1/messages"))

    def test_force_query_overrides_and_wins(self):
        pcfg = {"force_query_string": "beta=true&foo=bar"}
        self.assertEqual(
            "beta=true&foo=bar",
            ciel_runtime.upstream_messages_query(pcfg, "/v1/messages"),
        )
    ...
    def test_force_query_leading_question_mark_stripped(self):
        ...
        self.assertEqual("beta=1", ciel_runtime.upstream_messages_query(pcfg, "/v1/messages"))
    ...
    def test_non_anthropic_does_not_propagate_inbound_beta_by_default(self):
        for provider in ("vllm", "ollama-cloud", "opencode", "openrouter", "fireworks"):
            with self.subTest(provider=provider):
                self.assertEqual(
                    "",
                    ciel_runtime.upstream_messages_query({}, "/v1/messages?beta=true", provider),
                )
    ...
    def test_non_anthropic_force_query_still_wins(self):
        ...
        self.assertEqual(
            "beta=true",
            ciel_runtime.upstream_messages_query(pcfg, "/v1/messages?beta=false", "vllm"),
        )
...
class ForceQueryMenuTests(unittest.TestCase):
    def test_option_appears_in_anthropic_menu(self):
        rows, values = ciel_runtime.llm_option_panel_rows(
    ...
    def test_option_appears_in_non_anthropic_menus_with_empty_default(self):
        for provider in ("vllm", "ollama-cloud", "opencode"):
            with self.subTest(provider=provider):
                rows, values = ciel_runtime.llm_option_panel_rows(provider, {})
                self.assertIn("force_query_string", values)
                self.assertIn("empty", rows[values.index("force_query_string")])

    def test_ip_family_appears_only_for_routed_anthropic_menu(self):
        _rows, native_values = ciel_runtime.llm_option_panel_rows(
            "anthropic", {"route_through_router": False}
        )
        routed_rows, routed_values = ciel_runtime.llm_option_panel_rows(
            "anthropic", {"route_through_router": True}
        )
    ...
    def test_prompt_default_reflects_current_value(self):
        self.assertEqual(
            "",
            ciel_runtime.llm_option_prompt_default("anthropic", {}, "force_query_string"),
        )
        self.assertEqual(
            "beta=true",
            ciel_runtime.llm_option_prompt_default(
    ...
    def _run_menu_set(self, pcfg, value):
        ...
        with mock.patch.object(ciel_runtime, "load_config", lambda: cfg), \
             mock.patch.object(ciel_runtime, "save_config", lambda c: None), \
             mock.patch.object(ciel_runtime, "clear_model_cache", lambda: None):
            ciel_runtime.set_llm_option_config("anthropic", "force_query_string", value)
        return pcfg
    ...
    def test_menu_set_stores_value_for_ollama(self):
        ...
        with mock.patch.object(ciel_runtime, "load_config", lambda: cfg), \
             mock.patch.object(ciel_runtime, "save_config", lambda c: None), \
             mock.patch.object(ciel_runtime, "clear_model_cache", lambda: None):
            ciel_runtime.set_llm_option_config("ollama-cloud", "force_query_string", "trace=1")
