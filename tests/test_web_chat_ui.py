...
class WebChatUiTests(unittest.TestCase):
    ...
    def test_web_chat_posts_to_channel_bridge_and_streams_replies(self):
        ...
        self.assertIn("function renderMessageAttachments(attachments, bubble)", html)
        self.assertIn("function sameRuntimeAttachmentUrl(value, inline = false)", html)
        self.assertIn("safePreviewImageMime(mime)", html)
        self.assertIn("className = 'attachment-image'", html)
        self.assertIn("className = 'attachment-pdf'", html)
        self.assertIn("Preview safe source text", html)
        self.assertIn("renderMessageAttachments(attachments, bubble)", html)
        ...
        self.assertIn("The built-in CosyVoice sample is Chinese", html)
        self.assertIn('id="ttsAutoReference"', html)
        self.assertIn("captureAutoVoiceReference(chunks, sampleRate, transcriptText)", html)
        self.assertIn("Existing custom reference voices are never overwritten", html)
        self.assertIn("Deleting the saved reference automatically enrolls", html)
        self.assertIn("source === 'none'", html)
        self.assertIn("ASR timed out after 20 seconds", html)
        ...
        self.assertIn('id="colabForgetWorkerCredentials"', html)
        self.assertIn("encrypted Tailscale recovery key", html)
        self.assertIn("speechWorkerRecoveryMessage('ASR'", html)
        self.assertIn("Colab session may have stopped or its Tailscale tunnel may be disconnected", html)
        self.assertIn("job.failure_kind === 'colab_capacity'", html)
        self.assertIn("Tailscale rejected the saved recovery key", html)
