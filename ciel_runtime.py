...
def codex_launch_assembly() -> CodexLaunchAssembly:
    return CodexLaunchAssembly(
        config=CodexLaunchSharedConfigPorts(
            apply_launch_endpoint_policy, current_alias, current_launch_cwd_key,
            ensure_model_cache_for_launch, get_current_provider, load_config,
            provider_mode_label, record_launch_state_for_cwd,
            codex_runtime_model_catalog_args,
        ),
        installation=CodexLaunchSharedInstallationPorts(
            find_executable, install_codex_if_missing,
            warn_if_multiple_ciel_runtime_installs,
            disable_ciel_runtime_codex_prompts_for_native, has_passthrough_option,
            install_ciel_runtime_codex_prompts,
        ),
        dispatch=CodexLaunchSharedDispatchPorts(
            launch_agy, launch_claude, launch_codex, launch_codex_app_server,
            materialize_runtime_command, run_ciel_runtime_update_check,
            run_codex_update_check, run_prelaunch_menu,
            log_codex_passthrough_mapping,
        ),
        routing=CodexLaunchSharedRoutingPorts(
            cleanup_managed_services_for_provider, codex_routed_enabled,
            direct_native_codex_enabled, launch_readiness_errors,
            native_codex_enabled, codex_launch_enabled_for_provider,
            run_with_router_lifetime, start_router_if_needed,
        ),
        channel=CodexLaunchSharedChannelPorts(
            auto_import_passthrough_channels, channel_delivery_mode,
            codex_channel_capable_mcp_server_names, codex_mcp_native_http_compat_args,
            codex_mcp_split_proxy_enabled, restore_codex_mcp_config_from_managed,
            start_codex_mcp_channel_sse_for_launch,
            write_codex_mcp_config_for_channel_discovery, select_codex_resume_session,
        ),
        cli=CodexCliLaunchPorts(
            process=runtime_launch.CodexLaunchProcess(
                _channel_wake_enter_env_is_fixed,
                _codex_channel_wake_submit_delay_seconds,
                _codex_channel_wake_submit_retries, _log_codex_command_for_diagnostics,
                _set_channel_transcript_scope, codex_process_record_path,
                path_with_ciel_runtime_user_dirs, subprocess_call_with_channel_wake_proxy,
                terminate_existing_codex_processes_for_launch,
                terminate_existing_router_clients_for_launch,
            ),
            policy=runtime_launch.CodexLaunchCliPolicy(
                codex_alternate_screen_compat_args, codex_current_model_cli_args,
                codex_help_requested, codex_native_routed_config_args,
                codex_passthrough_args_for_launch, codex_passthrough_has_command,
                codex_resume_picker_requested, codex_resume_with_session_id,
                codex_runtime_config_args, codex_yolo_launch_args,
            ),
        ),
        app_server=CodexAppServerLaunchPorts(
            process=runtime_launch.CodexAppServerProcess(
                _log_codex_app_server_command_for_diagnostics,
                codex_process_record_path, path_with_ciel_runtime_user_dirs,
                subprocess_call_with_child_pid_record,
                terminate_existing_codex_processes_for_launch,
                terminate_existing_router_clients_for_launch,
            ),
            policy=runtime_launch.CodexAppServerCliPolicy(
                codex_app_server_default_listen_url, codex_app_server_launch_args,
                codex_current_model_config_args, codex_native_routed_config_args,
                codex_passthrough_has_model_override, codex_runtime_config_args,
                toml_string,
            ),
        ),
    )
